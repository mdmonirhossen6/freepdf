  ...
  return (
    <div className="mx-auto max-w-2xl py-10 text-center">
      <div className="rounded-lg border border-[var(--c-line-strong)] bg-[var(--c-panel)] p-8 dark:border-[var(--c-line)]">
        <h2 className="text-lg font-semibold text-[var(--c-fg)]">No matching resources found</h2>
        {query && (
        ...
        <div className="mt-5 text-left text-sm text-[var(--c-fg-muted)]">
          <p className="font-medium text-[var(--c-fg)]">Try:</p>
          <ul className="mt-1.5 list-inside list-disc space-y-1">
            <li>Checking the spelling</li>
            <li>Using a shorter, broader term</li>
            <li>Searching in Bengali or English (both work)</li>
            <li>Clearing the type or category filters</li>
          </ul>
        </div>
        <div className="mt-6 flex flex-wrap justify-center gap-2">
          ...
          <Link href="/browse" className="btn-ghost text-sm">
            Browse all {total.toLocaleString('en-US')} resources
          </Link>
        </div>
      </div>

      <div className="mt-8 text-left">
        <h3 className="text-sm font-medium text-[var(--c-fg)]">Popular resources instead</h3>
