  ...
  return (
    <header className="border-b border-[var(--c-line)] bg-[var(--c-bg)]">
      <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <h1 className="mt-4 text-2xl font-semibold tracking-tight text-[var(--c-fg)] sm:text-3xl">{meta.name}</h1>
        <p className="mt-2 max-w-2xl text-[var(--c-fg-muted)]">{meta.blurb}</p>
        <p className="mt-3 text-sm tabular-nums text-[var(--c-fg-subtle)]">
          {count.toLocaleString('en-US')} resources indexed
        </p>
      </div>
    </header>
  );
